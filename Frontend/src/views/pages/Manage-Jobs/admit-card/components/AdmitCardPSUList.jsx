...
const AdmitCardPSUList = ({ isActive }) => {
  ...
  // Fetch admit cards - PSU sector
  const fetchJobs = useCallback(async () => {
    setLoading(true);
    try {
      const params = getFilterParams(PAGE_SIZE);
      const res = await axios.get(`/job-categories/psu_admit_card_list`, { params });
      console.log("Fetched PSU admit cards:", res.data);
  ...
  useEffect(() => {
    ...
    const selectAll = document.getElementById('psu-admitcard-select-all');
    if (selectAll) {
  ...
  const columns = [
    {
      title: `<input type="checkbox" class="row-select-cb" id="psu-admitcard-select-all" />`,
      data: null,
      orderable: false,
      searchable: false,
    ...
    { title: "Job Title", data: "admitCard_JobId.job_title" },
        ...
        <Row>
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 pb-2 m-0 borderBottom">
            <DatatableActionButton
              endpoint="/job-categories/psu_admit_card_list"
              dataAccess="psuAdmitCardData"
            />
            <div className="">
              <TableFilters
            ...
            <DataTable
              ...
              options={{
                ...
                drawCallback: function () {
                  ...
                  if (!tableEl) return;
                  tableEl.removeEventListener("click", tableEl._psuAdmitCbHandler);
                  tableEl._psuAdmitCbHandler = (e) => {
                    const cb = e.target.closest('.row-select-cb');
                    if (!cb) return;
                    if (cb.id === 'psu-admitcard-select-all') {
                      toggleAllRows();
                      return;
                    }
                  ...
                  tableEl.addEventListener("click", tableEl._psuAdmitCbHandler);
                },
              }}
...
export default AdmitCardPSUList;
