...
const ResultsGovernmentList = ({ isActive }) => {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`/job-categories/government_result_list`, { params });
  ...
  useEffect(() => {
    ...
    const selectAll = document.getElementById('govt-result-select-all');
    if (selectAll) {
  ...
  const columns = [
    {
      title: `<input type="checkbox" class="row-select-cb" id="govt-result-select-all" />`,
      data: null,
      orderable: false,
      searchable: false,
        ...
        <Row>
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 pb-2 m-0 borderBottom">
            <DatatableActionButton
              endpoint="/job-categories/government_result_list"
            ...
            <DataTable
              ...
              options={{
                ...
                drawCallback: function () {
                  ...
                  if (!tableEl) return;
                  tableEl.removeEventListener("click", tableEl._govtResCbHandler);
                  tableEl._govtResCbHandler = (e) => {
                    const cb = e.target.closest('.row-select-cb');
                    if (!cb) return;
                    if (cb.id === 'govt-result-select-all') {
                      toggleAllRows();
                      return;
                    }
                  ...
                  tableEl.addEventListener("click", tableEl._govtResCbHandler);
                },
              }}
