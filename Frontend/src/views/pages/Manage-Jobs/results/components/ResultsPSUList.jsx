...
const ResultsPSUList = ({ isActive }) => {
  ...
  // Fetch results - PSU sector
  const fetchJobs = useCallback(async () => {
    setLoading(true);
    try {
      const params = getFilterParams(PAGE_SIZE);
      const res = await axios.get(`/job-categories/psu_result_list`, { params });
      console.log("Fetched PSU results:", res.data);
      const data = res.data?.jsonData;
      setJobs(data?.resultData || []);
  ...
  useEffect(() => {
    ...
    const selectAll = document.getElementById('psu-result-select-all');
    if (selectAll) {
  ...
  const columns = [
    {
      title: `<input type="checkbox" class="row-select-cb" id="psu-result-select-all" />`,
      data: null,
      orderable: false,
      searchable: false,
    ...
    { title: "Release Date", data: "result_ReleaseDate", render: (data) => formatDate(data) },
        ...
        <Row>
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 pb-2 m-0 borderBottom">
            <DatatableActionButton
              endpoint="/job-categories/psu_result_list"
              dataAccess="resultData"
            />
            <div className="">
              <TableFilters
            ...
            <DataTable
              ...
              options={{
                ...
                drawCallback: function () {
                  ...
                  if (!tableEl) return;
                  tableEl.removeEventListener("click", tableEl._psuResCbHandler);
                  tableEl._psuResCbHandler = (e) => {
                    const cb = e.target.closest('.row-select-cb');
                    if (!cb) return;
                    if (cb.id === 'psu-result-select-all') {
                      toggleAllRows();
                      return;
                    }
                  ...
                  tableEl.addEventListener("click", tableEl._psuResCbHandler);
                },
              }}
